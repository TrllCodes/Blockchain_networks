...
def test_transaction_exceeds_balance():
    with pytest.raises(Exception, match='Amount exceeds wallet balance'):
...
def test_transaction_update():
    ...
    to_first_recipient_again_amount = 25
    transaction.update(sender_wallet, first_recipient, to_first_recipient_again_amount)

    assert transaction.output[first_recipient] ==\
        first_amount + to_first_recipient_again_amount
    assert transaction.output[sender_wallet.address] ==\
        sender_wallet.balance - first_amount - next_amount - to_first_recipient_again_amount
    assert Wallet.verify(
        transaction.input['public_key'],
        transaction.output,
        transaction.input['signature']
...
def test_invalid_reward_transaction_extra_recipient():
    """
    This test ensures that only one wallet receives the mining reward for a
    newly mined block in the blockchain.
    """
    reward_transaction = Transaction.reward_transaction(Wallet())
...
def test_invalid_reward_transaction_invalid_amount():
    """
    This test ensures that the valid mining reward amount goes into the miner's
    wallet.
    """
    miner_wallet = Wallet()
